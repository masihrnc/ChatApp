...
export default function SetAvatar() {
  ...
  useEffect(() => {
    const localData = localStorage.getItem(import.meta.env.VITE_REACT_APP_LOCALHOST_KEY);
    if (!localData) navigate("/login");
  }, [navigate]);
...
//const   image = JSON.stringify(avatars[selectedAvatar])
     //console.log("data is" , image)
    // const { data } = await axios.post(`${setAvatarRoute}/${user._id}`, {
    //   image: avatars[selectedAvatar],
    // });
     const data = await fetch(`http://localhost:5000/api/login/setavatar/${userinfo._id}`, {
         method: "POST",
    ...
    if (data2.isSet) {
      ...
      userinfo.avatarImage = data2.image;
      //console.log("ocal data is " , userinfo)
